...
from model import dis_flow_model



def get_args():
	""" hyper-parameters. """
	parser = argparse.ArgumentParser('DAF model')
	...
	# Add model arguments
	parser.add_argument('--net_width', default=4, type=int, help='ratio between hidden layer width and input layer width in MADE')
	parser.add_argument('--net_depth', default=2, type=int, help='number of hidden layers in the MADE networks')
	parser.add_argument('--temperature', default=0.1, type=float, help='temperature in softmax to approxiamte argmax')
	parser.add_argument('--flow_number', default=2, type=int, help='number of MADE layers')
...
def main(args):
	...
	args.save_dir = 'flow'+args.save_dir+str(args.net_width)+' '+str(args.net_depth)+' '+str(args.flow_number)
	...
	for experiment in range(current_seed, args.experiment_num):
		...
		for epoch in range(last_epoch + 1, args.max_epoch):
			...
			model.eval()
			with torch.no_grad():
				likelihood = model(val.to(device))
				val_loss = -likelihood.mean(dim = 0)
