...
def get_args():
	""" hyper-parameters. """
	parser = argparse.ArgumentParser('SSVAT model')

	# Add data arguments
	parser.add_argument('--model', default = 'vae', help = 'use vae for training ssvat, use ae for training the AEA prior')
	...
	# Add model arguments
	parser.add_argument('--embedding_dimension', default=4, type=int, help='dimension of the embeddings')
	parser.add_argument('--visible_dimension', default=80, type=int, help='number of object classes')
	parser.add_argument('--num_heads', default=4, type=int, help='number of heads in attention')
	parser.add_argument('--num_encoders', default=1, type=int, help='number of transformer encoder layers')
	parser.add_argument('--num_decoders', default=1, type=int, help='number of transformer decoder layers')
	parser.add_argument('--dropout', default=0.1,type=float, help='dropout rate')
	parser.add_argument('--prior', default='gaussian',type=str, help='prior of the SSVAT')
...
def main(args):
	...
	# codes to fit the AEA prior are omitted, refer to that in the vae.py
	args.save_dir = 'transformer'+args.model+args.save_dir+str(args.embedding_dimension)+' '+str(args.num_encoders)+ ' ' + str(args.num_decoders
		) + args.prior + str(args.prior_hyper)
	...
	if os.path.isfile(checkpoint_path) and args.continue_last:
		...
		train_loader, val_loader, test_loader = utils.load_and_split_trans(args)
	...
	for experiment in range(current_seed, args.experiment_num):
		...
		for epoch in range(last_epoch + 1, args.max_epoch):
			...
			# padding index = 82
			loss_fn = nn.CrossEntropyLoss(ignore_index=args.visible_dimension+2, reduction='none')

			# Iterate over the training set
			for i, sample in enumerate(progress_bar):
				model.train()
				optimizer.zero_grad()
				indexs_forward, indexs_backward, permutation=utils.generate_indexs(args,sample.to(device))
				y_input_forward=indexs_forward[:,:-1]
				y_expected_forward = indexs_forward[:,1:]
				y_output_forward, kld_forward = model(indexs_forward, y_input_forward)
				if args.model=='vae':
					loss_forward = loss_fn(y_output_forward.permute(1,2,0),y_expected_forward).sum(dim=-1)+kld_forward
				else:
					loss_forward = loss_fn(y_output_forward.permute(1,2,0),y_expected_forward).sum(dim=-1)

				y_input_backward=indexs_backward[:,:-1]
				y_expected_backward = indexs_backward[:,1:]
				y_output_backward, kld_backward = model(indexs_backward, y_input_backward)
				if args.model=='vae':
					loss_backward = loss_fn(y_output_backward.permute(1,2,0),y_expected_backward).sum(dim=-1)+kld_backward
				else:
					loss_backward = loss_fn(y_output_backward.permute(1,2,0),y_expected_backward).sum(dim=-1)

				# calculate the mean probability and transfer it to the set probability
				loss=(-(torch.logsumexp(torch.cat((-loss_forward.unsqueeze(0),-loss_backward.unsqueeze(0)),dim=0),dim=0
					)-torch.tensor(2).log())-permutation).mean(dim=0)
				...
				stats['kl'] += (kld_forward + kld_backward).mean().detach().item()
			...
			with torch.no_grad():
				for sample_val in val_loader:
					indexs_forward, indexs_backward, permutation=utils.generate_indexs(args,sample_val.to(device))
					y_input_forward=indexs_forward[:,:-1]
					y_expected_forward = indexs_forward[:,1:]
					y_output_forward, kld_forward = model(indexs_forward, y_input_forward)
					if args.model=='vae':
